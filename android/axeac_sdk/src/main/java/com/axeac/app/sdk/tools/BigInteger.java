...
public class BigInteger {
	...
	private static final BigInteger ZERO = new BigInteger(0, ZERO_MAGNITUDE);
	private static final BigInteger ONE = valueOf(1);
	...
	/**
	 * -m ^（ - 1）mod b，b = 2 ^ 32（参见Montgomery mult。）
	 * */
	...
	public BigInteger(String sval, int rdx) throws NumberFormatException {
		...
		/*
		  ...
		  我们可以计算出需要存储的最大int数量
		  给定基数的sval.length数字，然后分配
		  存储在一个hit中，然后在一个hit中也产生大小
		 */
	...
	// describe:If axeac_sign >= 0, packs bytes into an array of ints, most significant first
	// 			If axeac_sign < 0, packs 2's complement of bytes into an array of ints, most
	// 			significant first, adding an extra most significant byte in case bval =
	// 			{0x80, 0x00, ..., 0x00}
	/**
	 * 如果axeac_sign> = 0，则将字节打包成一个int数组，
	 * <br>如果axeac_sign <0，则将2的字节补数打包成一个int数组，
	 * <br>在bval = {0x80， 0x00，...，0x00}的情况下添加一个额外的最高有效字节
	 */
	...
	public BigInteger(int sign, byte[] mag) throws NumberFormatException {
		if (sign < -1 || sign > 1) {
			throw new NumberFormatException("Invalid axeac_sign value");
		}
		if (sign == 0) {
			this.sign = 0;
	...
	// describe:strictly speaking this is a little dodgey from a compliance point of view
	...
	/**
	 * 它迫使人们使用SecureRandom，也就是说 - 这个实现是一个加密库，你需要有来源！
	 */
	...
	public BigInteger(int bitLength, int certainty, Random rnd) throws ArithmeticException {
		...
		if (bitLength == 2) {
			this.magnitude = rnd.nextInt() < 0 ? TWO.magnitude : THREE.magnitude;
			return;
		}
	...
	// describe:return a = a + b - b preserved.
	/**
	 * 返回a=a+b-b
	 */
	...
	private final static byte[] bitCounts = { 0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2,
	...
	private int bitLength(int indx, int[] mag) {
		...
		if (mag.length == 0) {
			...
			if (sign < 0) {
				...
				// 检查大小是否为2的幂
				boolean pow2 = ((bitCounts[mag[indx] & 0xff]) + (bitCounts[(mag[indx] >> 8) & 0xff])
	...
	// describe:unsigned comparison on two arrays - note the arrays may axeac_start with leading zeros.
	/**
	 * 两个数组间的无符号比较
	 */
	...
	// describe:return z = x / y - done in place (z value preserved, x contains the remainder)
	/**
	 * 返回z = x / y  ，完成（z值保留，x包含余数）
	 */
	...
	// describe:return whether or not a BigInteger is probably prime with a probability of 1 - (1/2)**certainty.
	/**
	 * 返回BigInteger是否可能是素数，概率为1 - （1/2）
	 */
	...
	// describe:Calculate the numbers u1, u2, and u3 such that:
	// 			u1 * a + u2 * b = u3
	 ...
	 * 计算数字u1，u2和u3，使得：
	 * <br>u1 * a + u2 * b = u3
	 * <br>其中u3是a和b的最大共同分隔符。 a和b使用
	 * <br>扩展欧几里得算法（参考“计算机艺术”第323页）
	 * <br>编程第2卷，第2版）
	 * @param a
	...
	public BigInteger modPow(BigInteger exponent, BigInteger m) throws ArithmeticException {
		...
		/*
		    ...
		    只有当模数为奇数时,Montgomery才能取幂，
            但是AFAIK，对于加密算法来说，一直是这样
		 */
		...
		if (useMonty) {
			...
			if (useMonty) {
				...
				if (zVal.length < m.magnitude.length) {
					...
					System.arraycopy(zVal, 0, longZ,
							longZ.length - zVal.length, zVal.length);
					zVal = longZ;
				}
			}
		}
	...
	// describe:return w with w = x * x - w is assumed to have enough space.
	/**
	 * 假设w = x * x-w返回w具有足够的空间。
	 */
	private int[] square(int[] w, int[] x) {
		// Note: this method allows w to be only (2 * x.Length - 1) words if result will fit
		// 如果有结果，该方法允许w只有（2 * x.Length - 1）个字
		long u1, u2, c;
	...
	// describe:return x with x = y * z - x is assumed to have enough space.
	/**
	 * 假设w = x * x-w返回w具有足够的空间。
	 */
	...
	// describe:Calculate mQuote = -m^(-1) mod b with b = 2^32 (32 = word size)
	/**
	 * 用b = 2 ^ 32（32 =word size）计算mQuote = -m ^（ - 1）mod b
	 */
	...
	// describe:Montgomery multiplication: a = x * y * R^(-1) mod m <br>
	// 			Based algorithm 14.36 of Handbook of Applied Cryptography. <br>
	// 			<li>m, x, y should have length n</li>
	// 			<li>a should have length (n + 1)</li>
	// 			<li>b = 2^32, R = b^n</li> <br>
	// 			The result is put in x <br>
	// 			NOTE: the indices of x, y, m, a different in HAC and in Java
	/**
	 * Montgomery乘法：a = x * y * R ^（ - 1）mod m <br>
	 * <br>基于应用密码学手册的算法14.36。点击
	 * <br><li> m，x，y长度n </ li>
	 * <br><li> a长度（n + 1）</ li>
	 * <br><li> b = 2 ^ 32，R = b ^ n </ li> <br>
	 * <br>结果放在x <br>中
	 * <br>注：x，y，m的索引，HAC和Java中的不同
	 */
	...
	private int remainder(int m) {
		...
		for (int pos = 0; pos < magnitude.length; ++pos) {
			acc = (acc << 32 | (magnitude[pos] & 0xffffffffL)) % m;
		}
		return (int) acc;
	}

	// describe:return x = x % y - done in place (y value preserved)
	/**
	 * 返回x = x％y ，完成（y值保留）
	 */
	...
	public BigInteger remainder(BigInteger n) throws ArithmeticException {
		...
		if (n.magnitude.length == 1) {
			...
			if (val > 0) {
				...
				return rem == 0 ? ZERO : new BigInteger(sign, new int[] { rem });
			}
		}
	...
	// describe:do a left shift - this returns a new array.
	/**
	 * 左移，返回新数组
	 */
	...
	// describe:do a right shift - this does it in place.
	/**
	 * 右移
	 */
	...
	// describe:do a right shift by one - this does it in place.
	/**
	 * 右移一位
	 */
	...
	// describe:returns x = x - y - we assume x is >= y
	/**
	 * 假定x> = y，返回x = x - y
	 */
	...
	public BigInteger xor(BigInteger val) {
		...
		int[] aMag = this.sign > 0 ? this.magnitude : this.add(ONE).magnitude;
		int[] bMag = val.sign > 0 ? val.magnitude : val.add(ONE).magnitude;
	...
	public String toString(int rdx) {
		...
		if (rdx == 16) {
			...
			/*
			   The axeac_sign is handled separatly.
			  ...
			  axeac_sign是分开处理的。
  			 基数16 _MUST_是一个特例，除非我们要输入一个递归。
  			 没有为BigIntegers采取了BigInteger作为参数是因为
  			 BigIntger是可克隆的，一些bouncycastle不是。）
  			 BigInteger u = new BigInteger(this.abs().toString(16), 16);
			 */
	...
	public int getLowestSetBit() {
		...
		int b = (word & 0x0000FFFF) == 0 ? (word & 0x00FF0000) == 0 ? 7 : 15
